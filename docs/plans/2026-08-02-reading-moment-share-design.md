...
The share sheet keeps the four type controls fixed at its top and the image/share actions fixed at its bottom. Only the bounded middle poster preview scrolls, so a reader can switch card type or act on the image from any point in a long poster. Switching card type resets the poster preview to its beginning. The fixed regions never cover the poster, and the sheet exposes no pixel dimensions, device names, production specifications, or explanatory filler around the preview. The AI composer remains hidden while the sheet is open.
...
Automated coverage verifies in-reading entry points, full primary and secondary content for all four card modes, dynamic poster height on long chapters, anonymity of life-manual cards, QR presence, downloadable PNG output, exact deep-link parameters, same chapter opening from a deep link, absence of selection-share UI, and no horizontal overflow at 320, 390, and 720 px. At each responsive width, the tests also verify that only the poster viewport scrolls, that tabs and actions keep their positions while it moves, and that changing card type returns the poster to the top. The normal 81-chapter, Silk B, H5, and iOS synchronization gates remain required.
