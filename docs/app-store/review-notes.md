...
Version 1.8 (17) updates AI dialogue to plain, thoughtful language inspired by Laozi while explicitly identifying it as AI. It also removes the in-app confirmation before saving a share image directly to Photos, and places brief save/copy feedback higher in the share panel. The first save still requests the required system Photos permission. The lifetime reading product has a lower localized price and continues to exclude AI services. Existing search, iPad support, conversation history, memory controls and purchase boundaries remain available.
