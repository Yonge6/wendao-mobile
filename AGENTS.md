...
- Keep a pressure-free `随喜相助 / Support the journey` entry at the very bottom of the More drawer on the public H5. It opens the supplied WeChat Pay code inside Wendao and must state clearly that support is optional: `有余则助，无余亦安`; reading, pausing, and sharing are already forms of participation. The payment image must retain native long-press interaction and provide a direct raw-image fallback for browsers that do not expose QR recognition. Hide this external-payment entry in native iOS builds unless it is replaced by an App Review-compliant in-app purchase flow.
