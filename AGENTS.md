...
- Keep `目录 / Contents` and `偶遇一章 / Chance` in the reading header. Use one accessible globe icon for whole-page Chinese/English switching and one search icon that opens the directory with its search field focused; do not bring back the wider `中 / EN` text pair.
...
- Keep each chapter’s explanation data in three layers (plain reading, philosophical structure, textual collation), but render textual collation only once in the original-text/version area; do not repeat it as a separate `校读 / Collation` item in `解释 / Meaning`. Keep `与你有关 / For you` as four life lenses (anxiety, relationships, choice, action), followed by verified personalization when available and one concrete daily practice.
- After the four life lenses, show one always-visible, chapter-specific `对我们的启发 / What this teaches us` item before the verified `人生说明书 / Life manual`. Maintain distinct original bilingual copy for all 81 chapters; do not generate it by merely swapping a theme word into one shared paragraph.
...
- Keep an `沿途所作 / Works along the way` section in the More drawer for the creator's related public works: 虾子曰、Human Design and Style Atlas. Present them as clearly marked external links with concise bilingual descriptions; do not confuse the external Human Design site with Wendao's in-product life manual.
- Keep a pressure-free `随喜相助 / Support the journey` entry at the very bottom of the More drawer on the public H5. It opens the supplied WeChat Pay code inside Wendao and must state clearly that support is optional: `有余则助，无余亦安`; reading, pausing, and sharing are already forms of participation. Match the proven Video Channel QR behavior: render the recognition image as a plain native `<img>` without draggable or custom touch/selection attributes, make the QR the dominant image content, and provide direct links to the standalone QR image and complete payment poster. Keep the modal close control fixed outside its scrolling figure. Hide this external-payment entry in native iOS builds unless it is replaced by an App Review-compliant in-app purchase flow.
